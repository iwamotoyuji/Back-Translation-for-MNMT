...
class Sampler():
    def __init__(self, src_DAMSM_RNN, tgt_DAMSM_RNN, netG, MMT, dataloader, opt):
        ...
        self.model_connector = ModelConnector(
            opt.batch_size, opt.eval_words_limit,
    ...
    def sampling_from_image_id(self, image_id):
        ...
        for i in range(self.opt.noise_num):
            ...
            if dataset.bpe_index2word is not None:
                ...
                src_inst = [dataset.src_word2index.get(word, Constants.UNK) for word in src_inst]
            tgt_inst = dataset.T2I_tgt_insts[inst_id]
            tgt_inst = [dataset.tgt_word2index.get(word, Constants.UNK) for word in tgt_inst]
            GAN_src_text, GAN_src_len = dataset.get_T2I_text(src_inst)
            GAN_tgt_text, GAN_tgt_len = dataset.get_T2I_text(tgt_inst)
            GAN_src_text = torch.LongTensor(GAN_src_text).unsqueeze(0).to(device)
            GAN_src_len = torch.LongTensor([GAN_src_len]).to(device)
            GAN_tgt_text = torch.LongTensor(GAN_tgt_text).unsqueeze(0).to(device)
            GAN_tgt_len = torch.LongTensor([GAN_tgt_len]).to(device)

            src_words_embs, src_sent_emb = self.src_DAMSM_RNN(GAN_src_text, GAN_src_len)
            src_mask = (GAN_src_text == 0)
            num_words = src_words_embs.size(2)
            if src_mask.size(1) > num_words:
                src_mask = src_mask[:, :num_words]

            tgt_words_embs, tgt_sent_emb = self.tgt_DAMSM_RNN(GAN_tgt_text, GAN_tgt_len)
            ...
            noise.data.normal_(0, 1)
            fake_imgs, _, _, _ = self.netG(
                noise, src_words_embs, tgt_words_embs, src_sent_emb, tgt_sent_emb, src_mask, tgt_mask)
